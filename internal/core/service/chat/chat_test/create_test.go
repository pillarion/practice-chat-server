...
func Test_service_CreateChat(t *testing.T) {
	...
	var (
		...
		usernames = []chatModel.Username{
			chatModel.Username(gofakeit.Username()),
			chatModel.Username(gofakeit.Username()),
			chatModel.Username(gofakeit.Username()),
		}
	...
	}{
		{
			...
			chatRepoMock: func(mc *minimock.Controller) chatRepo.Repo {
				mock := chatRepoMock.NewRepoMock(mc).
					InsertMock.Expect(ctxWithTx, usernames).
					Return(newID, nil)
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := srvc.CreateChat(tt.args.ctx, tt.args.username)
